...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def __init__(self, model: Type[ModelType]):
        ...
        self.exclude_columns = [self.model.create_time, self.model.update_time, self.model.is_delete]
        self.query_columns.extend((self.model.dt2ts(self.model.create_time, "created_ts"),
                                   self.model.dt2ts(self.model.update_time, "modified_ts")))
        self.init()
    ...
    def get(self, db: Session, _id: int, to_dict: bool = True) -> Optional[ModelType]:
        """ 通过id获取 """
        row = db.query(*self.query_columns).filter(self.model.id ==_id, self.model.is_delete == 0).first()
    ...
    def query(self, db: Session, *, queries: Optional[list] = None, filters: Optional[list] = None,
        ...
        """ 查询 """
        filters = (filters or []) + [self.model.is_delete == 0]
    ...
    def get_total(self, db: Session, *, queries: Optional[list] = None, filters: Optional[list] = None,
                  order_bys: Optional[list] = None, to_dict: bool = True) -> List[ModelType]:
        """
        查询所有数据
        :return data
    ...
    def update(self, db: Session, *, _id: int, obj_in: Union[UpdateSchemaType, Dict[str, Any]],
        ...
        obj = db.query(self.model).filter(self.model.id ==_id, self.model.is_delete != 1).update(update_data)
        db.commit()
        return obj

    def delete(self, db: Session, *, _id: int, deleter_id: int = 0) -> ModelType:
        """ 逻辑删除 """
        update_data = {self.model.is_delete: 1}
        if deleter_id:
            update_data[self.model.create_by] = deleter_id
        obj = db.query(self.model).filter(self.model.id == _id, self.model.is_delete != 1).update(update_data)
        db.commit()
        return obj
    ...
    def deletes(self, db: Session, *, _ids: List[int], deleter_id: int = 0) -> ModelType:
        ...
        if deleter_id:
            update_data[self.model.create_by] = deleter_id
        obj = db.query(self.model).filter(self.model.id.in_(_ids), self.model.is_delete != 1).update(update_data)
        db.commit()
        return obj
